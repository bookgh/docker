...
%{__install} -p -D %{SOURCE1} %{buildroot}%{prefix}
...
%post
mkdir /etc/my.cnf.d
mv %{prefix}/my.cnf /etc/my.cnf
chkconfig --add mysqld
chown -Rf %{MYSQL_GROUP}:%{MYSQL_USER} %{DATA_DIR}
%{prefix}/bin/mysql_install_db --basedir=%{prefix} --datadir=%{DATA_DIR} --user=%{MYSQL_USER} >/dev/null 2>&1
...
mv %{DATA_DIR}{,-`date +%m%d%H%M`} >/dev/null 2>&1
